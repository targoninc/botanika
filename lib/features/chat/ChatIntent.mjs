    ...
    static getTool() {
        return {
            tool: ToolBuilder.function("modifyChatHistory")
                .description("Modifies the chat history.")
            ...
            toolFunction: (text, context, parameters) => {
                if (parameters.action.type === "reset") {
                    return ResetChatAction.execute(text, context);
                } else if (parameters.action.type === "deleteLast") {
                    return DeleteLastAction.execute(text, context, parameters.action.value);
                }
            }
        };
    }
