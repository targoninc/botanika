...
export class TextToSpeechAction {
    static async getSpeech(requestId, text, context, canBeCached = false) {
        ...
        if (!canBeCached) {
            fileName = `req_${context.user.session}_${requestId}`;
        }
        ...
        if (!fileName.startsWith(`req_${context.user.session}`) && fs.existsSync(`.${url}`)) {
            return url;
        }
        ...
        const deleteTimeoutMinutes = 2;
        setTimeout(() => {
            fs.unlinkSync(`.${url}`);
        }, 1000 * 60 * deleteTimeoutMinutes);
