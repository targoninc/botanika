...
export class DatabaseIntent extends GenericIntent {
    ...
    static getTool() {
        return {
            tool: ToolBuilder.function("queryDatabase")
                .description("Query a database. Useful when data is explicitly needed that is likely to be stored in tables.")
                .tool,
            allowFollowupToolCalls: true,
            toolFunction: async (text, context, parameters) => {
                return await DatabaseIntent.execute(text, context);
            }
        }
    }
    ...
    static async execute(text, context) {
        ...
        if (!result || result.length === 0) {
            CLI.debug(`No data found for query ${query}`);
            return [];
        }
        await db.close();
        return [{
    ...
    static async improveQuery(query, db, e) {
        ...
        const messages = [
            {
                role: "system",
                content: `Improve the following query. Especially pay attention to indexes. 
                When working with indexes, make sure they're used correctly. 
                Only return the query string within markdown blocks, leaving out comments. Don't explain your thoughts. 
                The query should return the same data. If there is nothing to improve (except when there were no results previously), 
                just return the original query. Whatever you do, only return query text.`
            },
            {role: "system", content: "Column info: " + JSON.stringify(columnInfoMap)},
            {role: "system", content: "Index info: " + JSON.stringify(indexInfo)},
            e?.message ? [{role: "system", content: "You need to change the following query, because running it resulted in the following error: " + e.message}] : [],
            {role: "user", content: query}
        ].flat();

        const completionProvider = Completion.getProvider();
        const response = await Completion.complete(completionProvider, messages, "good", {
            temperature: 1
        });
