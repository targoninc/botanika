...
export class SpotifyApi {
    ...
    static runSpotifyAction(text, context, startTime) {
    ...
    static async getDevices(context) {
        try {
            const res = await axios({
                method: 'GET',
                url: `${spotifyConfig.baseUrl}/me/player/devices`,
    ...
    static async getContextTracks(contextUri, context) {
        try {
            const url = `https://api.spotify.com/v1/${contextUri.includes('album') ? "albums" : "playlists"}/${SpotifyApi.getContextId(contextUri)}/tracks`;
            const res = await axios({
                method: 'GET',
                url,
                headers: SpotifyApi.getDefaultHeaders(context)
