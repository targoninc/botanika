...
export class SpotifyApi {
    ...
    static async getContextTracks(contextUri, context) {
        try {
            const res = await axios({
                method: 'GET',
                url: `https://api.spotify.com/v1/${contextUri.includes('album') ? "albums" : "playlists"}/${SpotifyApi.getContextId(contextUri)}/tracks`,
                headers: SpotifyApi.getDefaultHeaders(context)
            });
            if (res.status === 200) {
    ...
    static getContextId(text) {
        const match = text.match(/(?:album|playlist)\/([a-zA-Z0-9]+)(?:\?|$)/);
