...
export class DatabaseIntent extends GenericIntent {
    ...
    static async getLikelyQuery(text, tablesWithDBs) {
        ...
        query that returns the requested data based on the existing tables. Only return the query, no explanations.
        If a table has a fulltext index, you can use the MATCH ... AGAINST syntax with BOOLEAN MODE. When using MATCH ... AGAINST, don't use wildcards.`;
        const completionProvider = ResponseAction.getCompletionProvider();
        const response = await completionProvider.api.chat.completions.create({
            messages: [
