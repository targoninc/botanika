...
export class GenericResponseAction {
    static async getOpenAiResponse(text, context) {
        const systemPrompt = "You are a virtual assistant. If the user says something that you don't understand, you just respond with an empty message. If the user says something that you do understand, you respond with a message that responds to that. Your response is allowed to be inappropriately rude or offensive, but only if the user is rude first or explicitly requests it.";
        const userPrompt = text;
        const openai = new OpenAI({
            apiKey: process.env.OPENAI_API_KEY,
        });
        const assistantInfo = `You are ${context.assistant.name}. Your language is ${context.assistant.language}. Prefer calling tool functions over responding, especially for saving info. Example: "I love my girlfriend" -> relationshipStatus = "in a relationship", "My favorite car is a Tesla" -> favoriteCar = "Tesla"`;
